    ...
    def get_neighbors(self, state, stone_weights):
        ...
        for action, dy, dx in directions:
            ...
            if state.maze[new_y][new_x] in ' .':
                ...
                new_maze[state.player_pos[0]][state.player_pos[1]] = ' ' if state.maze[state.player_pos[0]][state.player_pos[1]] == '@' else '.'
                new_maze[new_y][new_x] = '@' if new_maze[new_y][new_x] == ' ' else '+'
                
                new_cost_steps = state.cost_steps[:]
                new_cost_steps.append(state.path_cost + 1)
                
                new_state = MazeState(new_maze, (new_y, new_x), state.stone_positions, state, action.lower(), state.path_cost + 1, new_cost_steps)
                ...
                if state.maze[new_stone_y][new_stone_x] in ' .':
                    ...
                    new_state = MazeState(new_maze, (new_y, new_x), new_stone_positions, state, action, state.path_cost + stone_weights[stone_index] + 1, new_cost_steps)
                    neighbors.append(new_state)
